...
export default function CreateAccountWithGmail({ route, navigation }) {
    ...
    const [name, setName] = useState(route.params.userInfo.given_name + " " + route.params.userInfo.family_name)
    ...
    useEffect(() => {
        console.log('OPEN', CreateAccountWithGmail.name, 'SCREEN')

        return () => {
            console.log('SCREEN', CreateAccountWithGmail.name, 'CLOSE')
        }
    }, [])
